  ...
  return (
    <main className="min-h-screen bg-[var(--background)] text-saphir-navy">
      ...
      <section className="py-12 container mx-auto px-6">
        <div className="relative aspect-video bg-gray-50 rounded-[3rem] overflow-hidden border border-gray-100 group cursor-pointer shadow-2xl">
           <div className="absolute inset-0 bg-gradient-to-t from-saphir-navy/60 via-transparent to-transparent opacity-80"></div>
           ...
           <div className="absolute bottom-12 left-12 right-12 flex justify-between items-end">
              <div>
                ...
                <h2 className="text-4xl font-bold mb-2 text-white">Live Studio : Session Acoustique avec Sarah L.</h2>
                <div className="flex items-center gap-4 text-white/60 text-sm font-medium">
                   <span className="flex items-center gap-1"><Calendar size={14} /> 12 Mai 2026</span>
                   <span>• 45:12</span>
                </div>
              </div>
              <button className="flex items-center gap-2 text-white/60 hover:text-white transition-colors font-bold uppercase tracking-widest text-[10px]">
                Partager <ExternalLink size={16} />
              </button>
           </div>
        </div>
      </section>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="group cursor-pointer">
              ...
              <h4 className="font-bold text-lg leading-snug group-hover:text-saphir-electric transition-colors">
                Coulisses : Une journée type à la rédaction de Saphir FM 106.8
              </h4>
              <p className="mt-3 text-[10px] text-saphir-navy/30 uppercase font-bold tracking-widest italic">Reportage • il y a 3 jours</p>
            </div>
          ))}
        </div>
      </section>
