    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-20">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-3">
          ...
          <button className="flex items-center gap-2 px-6 py-3 bg-saphir-navy text-white rounded-2xl font-bold text-sm hover:bg-saphir-electric transition-all shadow-lg shadow-saphir-navy/10">
            <Save size={18} />
            Publier l'article
          </button>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
           ...
           <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm space-y-4">
             <label className="text-xs font-bold text-saphir-navy/40 uppercase tracking-widest">Image de couverture</label>
             <div className="aspect-video bg-gray-50 rounded-2xl border-2 border-dashed border-gray-100 flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-gray-100/50 transition-all group">
                ...
                <span className="text-[10px] font-bold text-gray-300 uppercase tracking-tighter">Cliquez pour uploader</span>
             </div>
           </div>
           ...
           <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm space-y-4">
              ...
              <div className="flex flex-col gap-2">
                 {['Brouillon', 'Public', 'Privé'].map((status) => (
                   <button 
                     key={status}
                   ...
                   >
                     {status}
                     {formData.status === status && <div className="w-2 h-2 rounded-full bg-saphir-electric"></div>}
                   </button>
                 ))}
              </div>
           </div>
